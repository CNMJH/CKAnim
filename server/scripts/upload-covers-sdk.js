...
// 用户提供的图片路径
const INPUT_IMAGE = '/home/tenbox/.copaw/media/om_x100b530b8ee324a0b294c0fe808d787_img_v3_02104_97b79851-1b49-4b14-9d8d-a6314716eebg.jpg';
 ...
 * 使用七牛云 SDK 上传文件
 */
async function uploadToQiniu(filePath, key) {
...
async function main() {
  ...
  for (const video of yelanVideos) {
    ...
    try {
      ...
      const [jpgResult, webpResult] = await Promise.all([
        uploadToQiniu(jpgPath, jpgKey),
        uploadToQiniu(webpPath, webpKey),
      ]);
      
      // 保存 URL 用于后续更新数据库
      video.coverUrl = jpgResult.url;
      video.coverUrlJpg = jpgResult.url;
      video.coverUrlWebp = webpResult.url;
  ...
  // 输出 SQL 更新语句
  console.log(`\n${'='.repeat(60)}`);
  console.log('📝 数据库更新 SQL 语句');
  console.log(`${'='.repeat(60)}\n`);
  
  for (const video of yelanVideos) {
    if (video.coverUrl) {
      const sql = `UPDATE videos SET coverUrl = '${video.coverUrl}', coverUrlJpg = '${video.coverUrlJpg}', coverUrlWebp = '${video.coverUrlWebp}' WHERE id = ${video.id};`;
