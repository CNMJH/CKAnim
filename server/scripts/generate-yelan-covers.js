...
/**
 * 为夜兰的 3 个视频生成封面图并上传到七牛云
 * 使用 curl 直接上传，绕过 SDK 区域查询问题
 */
...
async function main() {
  ...
  for (const video of yelanVideos) {
    ...
    try {
      ...
      execSync(`ssh -i /home/tenbox/.ssh/ckanim_deploy -o StrictHostKeyChecking=no root@39.102.115.79 "cd /var/www/ckanim/server && sqlite3 prisma/dev.db \\"${updateSql}\\""`, {
        stdio: 'inherit',
      });
